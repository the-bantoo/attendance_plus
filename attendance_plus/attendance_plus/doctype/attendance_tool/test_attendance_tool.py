QUnit.module('attendance_plus');

QUnit.test("Test: Attendance tool [Attendance_plus]", function (assert) {
	assert.expect(2);
	...
	frappe.run_serially([
		...
		() => frappe.set_route("Form", "Attendance Tool"),
		() => frappe.timeout(0.5),
		() => assert.equal("Attendance Tool", cur_frm.doctype,
			"Form for Attendance Tool opened successfully."),
		// set values in form
